...
export const CommentSection = ({ postId }: Props) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (filter.isProfane(newCommentText)) {
  ...
  return (
    <div className="mt-6">
      ...
      {user ? (
        <form onSubmit={handleSubmit} className="mb-4">
          <textarea
            ...
            className="w-full border border-white/10 bg-slate-200 p-2 rounded"
            placeholder="Write a comment..."
            rows={3}
          />
          <button
            type="submit"
            className="mt-2 bg-amber-500 text-white px-4 py-2 rounded cursor-pointer hover:bg-rose-600 transition duration-200"
          >
      ...
      >
        Profanity is not allowed in comments.
      </div>
    </div>
  );
};